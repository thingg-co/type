...
- Release builds are signed only when BOARD_KEYSTORE/BOARD_KEYSTORE_PASS/BOARD_KEY_PASS are set
...
- No unit test suite yet; verify changes by driving the emulator (adb `input tap`, `screencap`,
  logcat tags BoardIME, SpellLlm, BoardLLM, Dictionary). Pace scripted taps ~250 ms apart.
...
- `dict/` - 52k-word frequency list + trie (assets/en_words.txt): known-word gate, bounded
  Damerau-Levenshtein suggestions, adaptive-key weights.
- `ime/` - `BoardInputMethodService` (word tracking, boundary autocorrect with undo, sentence
