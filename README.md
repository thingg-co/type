...
- While you type, the bar is context-aware: a lexer reads the sentence so far and a 36 MB
  on-device recurrent network (two GRU layers of 512 over the whole sentence, 192-d tied
  embeddings, int8 output, trained on 93M words by tools/nn/, a third of them film and TV
  dialogue) suggests the next word after each space and ranks completions ("should ha" puts
  "have" first). On held-out news and encyclopedic text it hits top-3 next-word 41.4% vs 19.7%
  for the bigram table it replaced, and on held-out dialogue 36.2% vs 16.8%; the bigram table
  stays as an instant fallback. If what you typed cannot start
